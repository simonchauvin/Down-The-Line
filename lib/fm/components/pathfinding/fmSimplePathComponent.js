...
FMENGINE.fmSimplePathComponent = function (pOwner) {
    ...
    var that = FMENGINE.fmComponent(FMENGINE.fmComponentTypes.PATHFINDING, pOwner),
         ...
         */
        desiredSpeed = FMENGINE.fmPoint(0, 0),
        /**
         * Speed at which the game object follow the path.
         */
        actualSpeed = FMENGINE.fmPoint(0, 0),
        /**
         ...
         */
        positionBeforeStopping = FMENGINE.fmPoint(0, 0),
        /**
    ...
    that.startFollowingPath = function (pSpeed, pIndexToStartFrom) {
        if (waypoints.length > 0) {
            ...
            var xDiff =  Math.abs((spatial.x + physic.offset.x + physic.width / 2) - waypoints[currentIndex].x),
                yDiff =  Math.abs((spatial.y + physic.offset.y + physic.height / 2) - waypoints[currentIndex].y),
                coeff;
            if (xDiff < yDiff) {
                coeff = xDiff / yDiff;
    ...
    that.resumeFollowingPath = function () {
        if (waypoints.length > 0) {
            active = true;
            if (positionBeforeStopping.x !== spatial.x
                    || positionBeforeStopping.y !== spatial.y) {
                xReached = false;
                yReached = false;
                //Adjust speed so that the movement is linear
                var xDiff =  Math.abs((spatial.x + physic.offset.x + physic.width / 2) - waypoints[currentIndex].x),
    ...
    that.stopFollowingPath = function () {
        ...
        physic.velocity.x = 0;
        physic.velocity.y = 0;
        positionBeforeStopping = FMENGINE.fmPoint(spatial.x, spatial.y);
    };
